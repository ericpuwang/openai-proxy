...
export function makeReadableWebSocketStream(
  ...
  return new ReadableStream<ArrayBuffer>({
    start(controller) {
      ...
      ws.addEventListener("close", () => {
        try {
          ...
          console.log(`websocketStream can't close DUE to `, error);
        }
      });
      // header ws 0rtt
      const { earlyData, error } = base64ToArrayBuffer(earlyDataHeader);
      if (error) {
        console.log(`earlyDataHeader has invaild base64`);
        closeWebSocket(ws);
        return;
      }
      if (earlyData) {
    ...
    cancel(reason) {
      ...
      console.log(`websocketStream is cancel DUE to `, reason);
      if (readableStreamCancel) {
        return;
      }
      readableStreamCancel = true;
...
export function processVlessHeader(
  ...
  let isUDP = false;
  if (uuid.stringify(new Uint8Array(vlessBuffer.slice(1, 17))) === userID) {
    isValidUser = true;
  }
  if (!isValidUser) {
    return {
